...
namespace OAuthTokenForGoogleCalendar
{
    public class DatabaseDataStore : IDataStore
    {
        ...
        public DatabaseDataStore(int orgId,string clientKey,string secretKey)
        {
        ...
        public System.Threading.Tasks.Task StoreAsync<T>(string key, T value)
        {
            ...
            if (val != null)
            {
                ...
                {
                    UpdateRefreshToken(OrgId, (string)val,ClientKey,SecretKey);
                }
            }
        ...
        public void UpdateRefreshToken(int orgId, string refreshToken,string clientKey,string secretKey)// string clientKey, string clientSecretKey,string refreshToken)
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    try
                    {
                        using (SqlCommand cmd = new SqlCommand("UpdateCalendarRefreshToken", con))
                        {
                            ...
                            cmd.Parameters.Add("@SecretKey", SqlDbType.VarChar).Value = secretKey;                            
