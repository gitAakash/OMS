...
namespace CampaignTrackEmailDownloader
{
    ...
    //        GoogleCalSvc.CheckCalendarMovements();
        ...
        public string ProcessData()
        {
            ...
            try
            {
                ...
                foreach (string file in Directory.EnumerateFiles(FullPath, "*.htm"))
                {
                        ...
                        for (int j = 0; j < ds.Tables[1].Rows.Count; j++)
                        {
                            ...
                            if (dsProductNotification.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (dsProductNotification.Tables[0].Rows[dsProductNotification.Tables[0].Rows.Count - 1]["Value"].ToString().Contains("@"))
                                {
                                    ...
                                    eml.SendMail(Subject, htmlDoc.DocumentNode.OuterHtml, "gade.swapnil@gmail.com", "", false);
                    ...
                    {
                        eml.SendMail("No Product Match..", "The file name is " + filename, "support@zerofootprint.com.au", "", false);
            ...
            catch (Exception ex)
            {
                ...
                //{
                    eml.SendMail("Error in parsing file..", "The file name is " + filename + ". Error :- " + strLog, "support@zerofootprint.com.au", "", false);
                    WriteLog(strLog + "For Filename :" + filename);
                    File.Move(System.Configuration.ConfigurationSettings.AppSettings["MailPath"] + filename, System.Configuration.ConfigurationSettings.AppSettings["Error"] + filename);
                    filename = filename.Replace("htm", "txt");
                    File.Move(System.Configuration.ConfigurationSettings.AppSettings["MailPath"] + filename, System.Configuration.ConfigurationSettings.AppSettings["Error"] + filename);
        ...
        private void ProprtyCoordinates(int propertyId, string location)
        {
            try
            {
                CampaignTrack_MailScarapper.GoogleService.IGoogleNotificationService google = new CampaignTrack_MailScarapper.GoogleService.GoogleNotificationServiceClient();
        ...
        DataSet ExecuteProcess(string XML)
        {
            try
            {
                string connectionString = null;
